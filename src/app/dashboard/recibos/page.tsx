  ...
  const fetchRecibos = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError('Tabela de recibos ainda não foi criada no banco. Aguarde alguns minutos e recarregue.')
      } else {
        setError('Erro ao carregar recibos')
      }
    } catch (error) {
      console.error('Erro ao buscar recibos:', error)
      setError('Erro de conexão. Verifique se o sistema está funcionando.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Recibos Gerados</h1>
              ...
              <div>
                ...
                {error.includes('tabela') && (
                  <div className="mt-2 text-xs text-red-600">
                    <p><strong>Solução:</strong> O sistema está aplicando as mudanças do banco. Aguarde 2-3 minutos e clique em "Atualizar Lista".</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {recibos.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {error ? 'Não foi possível carregar recibos' : 'Nenhum recibo encontrado'}
              </h3>
              <p className="text-gray-600">
                {error 
                  ? 'Verifique se o sistema foi deployado corretamente.' 
                  : 'Recibos são gerados automaticamente quando pagamentos são marcados como pagos.'}
              </p>
              {!error && (
                <div className="mt-4 text-sm text-gray-500">
                  <p><strong>Para gerar um recibo:</strong></p>
                  <p>1. Vá em Pagamentos</p>
                  <p>2. Marque um pagamento como "Pago"</p>
                  <p>3. O recibo aparecerá aqui automaticamente</p>
                </div>
              )}
            </div>
          ) : (
